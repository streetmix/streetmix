...
describe('DateTimeRelative', () => {
  ...
  it('renders "yesterday at {time}" string', () => {
    // Pass in timezone as UTC to force it not to use server's time zone
    const { container } = render(
      <DateTimeRelative value="2018-04-22T18:00:00.000Z" timezone="UTC" />
    )
    expect(container).toHaveTextContent('Yesterday at 6:00 PM')
  })

  it('renders a date that happened this year', () => {
    const { container } = render(
      <DateTimeRelative value="2018-02-03T18:00:00.000Z" timezone="UTC" />
    )
  ...
  it('renders a date in another year', () => {
    const { container } = render(
      <DateTimeRelative value="2017-10-17T18:00:00.000Z" timezone="UTC" />
    )
