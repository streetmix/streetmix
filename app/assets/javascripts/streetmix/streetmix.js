...
"use strict";
  ...
  function _saveStreetToServer(initial) {
    ...
    var postUrl = API_URL + 'v2/streets';
    if (street.hasOwnProperty('id') && typeof(street.id) != 'undefined') postUrl += '/' + street.id;
  ...
  function _saveSettingsToServer() {
    ...
    _newNonblockingAjaxRequest({
      // TODO const
      url: API_URL + 'v2/users/' + signInData.userId,
      data: transmission,
      dataType: 'json',
      type: 'PUT',
      contentType: 'application/json',
  ...
  function _fetchLastStreet() {
    _newBlockingAjaxRequest(msg('LOADING'),
        {
          // TODO const
          url: API_URL + 'v2/streets/' + settings.priorLastStreetId,
          dataType: 'json',
          type: 'GET',
          headers: { 'Authorization': _getAuthHeader() }
  ...
  function _fetchGalleryData() {
    if (galleryUserId) {
      $.ajax({
        // TODO const
        url: API_URL + 'v2/users/' + galleryUserId + '/streets',
        dataType: 'json',
        type: 'GET',
        headers: { 'Authorization': _getAuthHeader() }
      ...
      $.ajax({
        // TODO const
        url: API_URL + 'v2/streets?count=200',
        dataType: 'json',
        type: 'GET'
      }).done(_receiveGalleryData).fail(_errorReceiveGalleryData);
    }
  ...
  function _fetchGalleryStreet(streetId) {
    ...
    $.ajax({
      // TODO const
      url: API_URL + 'v2/streets/' + streetId,
      dataType: 'json',
      type: 'GET',
      headers: { 'Authorization': _getAuthHeader() }
  ...
  function _sendSignOutToServer(quiet) {
    var call = {
      // TODO const
      url: API_URL + 'v2/users/' + signInData.userId + '/login-token',
      dataType: 'json',
      type: 'DELETE',
      headers: { 'Authorization': _getAuthHeader() }
    };
  ...
  function _getFetchStreetUrl() {
    // TODO const
    if (street.creatorId) {
      var url = API_URL + 'v2/streets?namespacedId=' +
