...
var TWITTER_ID_COOKIE = 'twitter_id';
...
function _removeSignInCookies() {
  ...
  $.removeCookie(TWITTER_ID_COOKIE);
}
...
function _loadSignIn() {
  ...
  var twitterIdCookie = $.cookie(TWITTER_ID_COOKIE);

  if (signInCookie && userIdCookie) {
    signInData = { token: signInCookie, userId: userIdCookie, twitterId: twitterIdCookie };
  ...
  if (signInData && signInData.token && signInData.twitterId) {
    _fetchSignInDetails();
...
function _fetchSignInDetails() {
  // TODO const
  $.ajax({
    url: API_URL + 'v2/users/' + signInData.twitterId,
    dataType: 'json',
...
function _getAuthHeader() {
  if (signInData && signInData.token) {
    return 'Token token="' + signInData.token + '"'
  } else {
    return '';
  }
}
...
function _sendSignOutToServer(quiet) {
  var call = {
    // TODO const
    url: API_URL + 'v2/users/' + signInData.twitterId + '/destroy-api-auth-token',
    dataType: 'json',
    type: 'DELETE',
    headers: { 'Authorization': _getAuthHeader() }
  };
...
function _signInLoaded() {
  ...
  if (signedIn) {
    document.querySelector('#gallery-link a').href = '/' + signInData.twitterId;
  }
