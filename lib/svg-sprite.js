...
function compileSVGSprites (source, filename, namespace) {
  ...
  glob(pattern, function (error, files) {
    ...
    logger.info(
      chalk`[svg-sprite] {cyan Compiling SVG sprites from} ${pattern} {cyan ...}`
    )
    ...
    files.forEach(function (file) {
      ...
      const svg = new Vinyl({
        base: path.join(process.cwd(), pattern).split('*')[0],
      ...
      const id = `${namespace}-${svg.relative
        ...
        .replace(/.svg$/, '')}`
      logger.verbose(
        chalk`[svg-sprite] {cyan Compiling} {gray ${file}} {cyan as} {white ${id}}`
      )
    })
