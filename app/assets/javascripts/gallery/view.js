...
function _receiveGalleryData(transmission) {
  ...
  for (var i in transmission.streets) {
    ...
    /*if (!galleryTwitterId && (galleryStreet.data.undoStack.length <= 4)) {
      anchorEl.classList.add('virgin');
    }*/

    galleryStreet.creatorId =
        (galleryStreet.creator && galleryStreet.creator.twitterId);
    ...
    if (signedIn && (galleryStreet.creatorId == signInData.twitterId)) {
...
function _showGallery(twitterId, instant, signInPromo) {
  if (readOnly) {
    return;
  }
...
>>>>>>> master:assets/js/gallery/view.js
  ...
  galleryTwitterId = twitterId;

  if (signInPromo) {

  } else {
    if (twitterId) {
      document.querySelector('#gallery .avatar').setAttribute('twitterId', galleryTwitterId);
      document.querySelector('#gallery .avatar').removeAttribute('loaded');
      _fetchAvatars();
      document.querySelector('#gallery .user-id').innerHTML = galleryTwitterId;

      var linkEl = document.createElement('a');
      // TODO const
      linkEl.href = 'https://twitter.com/' + galleryTwitterId;
    ...
    } else if (signedIn && (twitterId == signInData.twitterId)) {
      document.querySelector('#gallery').classList.remove('another-user');
...
function _onMyStreetsClick(event) {
  ...
  if (signedIn) {
    _showGallery(signInData.twitterId, false);
  } else {
