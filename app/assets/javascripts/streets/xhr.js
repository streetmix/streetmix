...
function _getFetchStreetUrl() {
  // TODO const
  if (!!street.creator && (!!street.creator.userId || !!street.creator.twitterId)) {
    var creatorId = street.creator.userId || street.creator.twitterId;
...
function _saveStreetToServer(initial) {
  ...
  if (initial) {
    ...
    $.ajax({
      // TODO const
      url: API_URL + 'v2/streets/' + street.id,
      data: transmission,
      dataType: 'json',
      type: 'PUT',
...
function _unpackServerStreetData(transmission, id, namespacedId, checkIfNeedsToBeRemixed) {
  ...
  if (checkIfNeedsToBeRemixed) {
    if (!signedIn || (street.creator.userId != signInData.userId)) {
