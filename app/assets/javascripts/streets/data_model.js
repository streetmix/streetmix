...
function _setStreetCreator(newUserId, newTwitterId) {
  street.creator = {
    userId: newUserId,
...
function _getStreetUrl(street) {
  var url = '/';
  if (street.creator && street.creator.twitterId) {
    if (RESERVED_URLS.indexOf(street.creator.twitterId) != -1) {
      url += URL_RESERVED_PREFIX;
    }

    url += street.creator.twitterId;
  } else {
    url += URL_NO_USER;
  }
